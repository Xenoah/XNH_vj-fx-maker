...
- [ ] CSS を AE 風 5 ペイン (Top / Left / Center / Right / Bottom) に再構築
- [ ] 全 50 効果に `cat` (カテゴリ) 付与: Transform / Color / Light / Blur / Distort / Glitch / Time
- [ ] 左パネルにカテゴリ別折畳式エフェクトライブラリ
- [ ] 右パネルは「選択中レイヤのコントロール」専用に
- [ ] 下パネルにタイムライン枠 (中身は Phase 2)
- [ ] 上ツールバーに BPM/録画/全画面/プリセット集約
- [ ] **E10**: 左/右/下パネルの境界をドラッグでリサイズ

### Phase 1 — レイヤースタック & 効果選択  ⏳
- [ ] エフェクトを「レイヤー」概念に変更 (同じ効果を複数追加可能)
- [ ] ライブラリの効果クリックで「+ 追加」ボタン → レイヤー追加
- [ ] レイヤー一覧 (タイムラインの縦軸) で選択状態管理
- [ ] 右ペインに選択レイヤのパラメータ詳細表示
- [ ] レイヤー削除/複製/並び替え/有効化トグル
- [ ] **E9**: 効果の複製 (同じレイヤをコピー)
