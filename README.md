...
- **ランダム**: 3〜7 個の効果を無作為に有効化、各パラメータを乱数化
- **クリア**: 全効果 OFF + 既定値に戻す
...
- **パネル隠す** ボタン or `h` キー: 左右パネルを隠してプレビューを最大化
...
- プリセット JSON はバージョン互換を維持するため `id` ベースで照合します。未知の id は無視されます。
